...
class ImportLatLongInternetData implements OnEachRow, WithEvents
{
    ...
    private array $batch_internet_data;
...
// set TRUE;
        ...
        echo 'Internet Lat-Long Data : ' . \App\Models\ImportLatLongInternetData::upsert($importer_this->batch_internet_data, ['name', 'latitude', 'longitude'], [
    ...
    public function onRow(Row $row)
    {
        ...
        if ($rowIndex > $this->heading_row_number) {
            ...
            if (!empty($data['latitude']) && !empty($data['longitude']) && !empty($data['name'])) {
                $this->batch_internet_data[$this->j] = $data;
                $this->j++;
            }
        }
    }
